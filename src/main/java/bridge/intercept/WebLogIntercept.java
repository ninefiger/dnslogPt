...
public class WebLogIntercept implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        String isAuth = "0";
        //增加自定义header限制
            //先判断用户启动时是否配置了headerSignal;
        if (DnslogConfig.headerSignal == null){
            isAuth = "1";
        }else{
            String privacyHeader = DnslogConfig.privacyHeader;
            String headerSignal = request.getHeader(privacyHeader);
            if (headerSignal != null && headerSignal.equals(DnslogConfig.headerSignal)){
                isAuth = "1";
            }
        }
        String logID;
